...
#define _log(LEVEL) L::stream(LEVEL, L::DETAIL::MakeFileBlock<c_run_time::StringLength(L::DETAIL::GetFileName(__FILE__)), c_run_time::StringLength(CS_STRINGIFY(__LINE__))>(L::DETAIL::GetFileName(__FILE__), CS_STRINGIFY(__LINE__)).Get())
#define L_PRINT(LEVEL) L::stream(LEVEL, "[" __FILE_NAME__ ":" CS_STRINGIFY(__LINE__) "] ")
#else
#define _log(LEVEL) L::stream(LEVEL, L::DETAIL::MakeFileBlock<c_run_time::StringLength(L::DETAIL::GetFileName(__FILE__)), c_run_time::StringLength(CS_STRINGIFY(__LINE__))>(L::DETAIL::GetFileName(__FILE__), CS_STRINGIFY(__LINE__)).Get())
#define L_PRINT(LEVEL) L::stream(LEVEL, L::DETAIL::MakeFileBlock<c_run_time::StringLength(L::DETAIL::GetFileName(__FILE__)), c_run_time::StringLength(CS_STRINGIFY(__LINE__))>(L::DETAIL::GetFileName(__FILE__), CS_STRINGIFY(__LINE__)).Get())
#endif
#else
#define L_PRINT(LEVEL) L::stream(LEVEL)
#define _log(LEVEL) L::stream(LEVEL)
#endif
#pragma endregion
	...
	struct Stream_t
	{
		...
		template <typename T> requires std::is_integral_v<T>
			Stream_t& operator<<(const T value)
			{
				...
				char szIntegerBuffer[c_run_time::IntegerToString_t<std::int64_t, 2U>::MaxCount() + 2U];
				char* szInteger = c_run_time::IntegerToString(value, szIntegerBuffer + 2U, sizeof(szIntegerBuffer) - 2U, iBase);
			...
			template <typename T> requires std::is_floating_point_v<T>
				Stream_t& operator<<(const T value)
				{
					...
					*szFormat = '\0';
					...
					const int nFloatLength = c_run_time::StringPrintN(szFloatBuffer, sizeof(szFloatBuffer), szFormatBuffer, iDesiredPrecision, value);
				...
				Stream_t& operator<<(const Vector_t& vecValue)
				{
					...
					this->iPrecision = 3;
					*this << xorstr_("vector3d: (") << vecValue.x << xorstr_(" | ") << vecValue.y << xorstr_(" | ") << vecValue.z << xorstr_(")");
					return *this;
				}
				...
				Stream_t& operator<<(const QAngle_t& angValue)
				{
					this->nModeFlags |= LOG_MODE_FLOAT_FORMAT_FIXED;
					this->iPrecision = 3;
					*this << xorstr_("qangle: (") << angValue.x << xorstr_(" | ") << angValue.y << xorstr_(" | ") << angValue.z << xorstr_(")");
					return *this;
				}
