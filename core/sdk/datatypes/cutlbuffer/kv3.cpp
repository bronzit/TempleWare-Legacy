...
KeyValues3* KeyValues3::set_type(kv_basic_type a1, kv_basic_type a2)
{
	...
	static auto setType = *reinterpret_cast<fnTypeData*>(MEM::FindPattern(CLIENT_DLL, xorstr_("48 8B C4 55 48 8D A8 08 FD")));
	
#ifdef CS_PARANOID
	debug(setType != nullptr);
#endif
...
KeyValues3* KeyValues3::create_object(const char* name, bool unk1)
{
	...
	static CreateObjectFn createObjectFn = *reinterpret_cast<CreateObjectFn*>(MEM::FindPattern(CLIENT_DLL, xorstr_("48 89 5C 24 08 48 89 74 24 10 57 48 81 EC E0 00 00 00 48")));
	debug(createObjectFn != nullptr);
...
void KeyValues3::set_data(const char* name, const char* value, kv_field_type_t field)
{
	...
	static auto setData = reinterpret_cast<fnSetData>(MEM::FindPattern(CLIENT_DLL, xorstr_("48 89 5C 24 ?? 48 89 6C 24 ?? 56 57 41 54 41 56 41 57 48 83 EC 30 0F B6 01 45 0F B6 F9 8B 2A 4D 8B E0 4C 8B 72 08 48 8B F9")));

#ifdef CS_PARANOID
	debug(setData != nullptr);
#endif
...
void KeyValues3::set_data(const char* name, uint32_t color)
{
	...
	static auto setData = reinterpret_cast<fnSetData>(MEM::FindPattern(PARTICLES_DLL, xorstr_("E8 ?? ?? ?? ?? 49 C1 E5 21")));
...
void KeyValues3::set_data(const char* name, int value)
{
	...
	static auto setData = reinterpret_cast<fnSetData>(MEM::FindPattern(CLIENT_DLL, xorstr_("")));
